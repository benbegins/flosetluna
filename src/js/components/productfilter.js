...
const productFilter = () => {
    ...
    const categorySelect = document.querySelector('.archive-product__cat__select');
    const categorySelectContainer = document.querySelector('.archive-product__cat__select-container');
    const categoryLink = document.querySelectorAll('.archive-product__cat__link');
    ...
    if (productContainer) {
        ...
        window.addEventListener('scroll', () => {
            if ((window.scrollY + headerHeight) > catPosition && window.innerWidth >= 1024) {
                categoryList.classList.add('is-fixed');
                categoryList.style.top = (headerHeight + siteHeader.offsetTop) + "px";
                productContainer.style.marginTop = categoryList.offsetHeight + "px";
            } else if (window.scrollY > 200 && window.innerWidth <= 1024) {
                categorySelectContainer.style.opacity = 1;
                categorySelectContainer.style.top = (headerHeight + siteHeader.offsetTop) + 'px';
            } else {
